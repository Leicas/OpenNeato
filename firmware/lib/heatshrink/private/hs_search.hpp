...
namespace heatshrink {
    ...
    public:
        ...
        byte_span& operator=(const byte_span&) noexcept = default;
    ...
    private:
        ...
        template<uint32_t M, uint32_t N = 0>
        static bool __attribute__((always_inline)) unrolled_find_3(const uint8_t *& data, const uint32_t vl,
                                                                   const uint32_t vh) noexcept {
            static_assert(N < M);
            if constexpr (N % 2 == 0) {
        ...
        template<uint32_t M, uint32_t N = 0>
        static bool __attribute__((always_inline)) unrolled_comp_f_l(const uint8_t *& first, const uint8_t *& last,
                                                                     const uint32_t f, const uint32_t l) noexcept {
            ...
            do {
                {
                    ...
                    while (first < end_unrolled && !unrolled_comp_f_l<LOOP_UNROLL_FACTOR>(first, last, f, l))
                            [[likely]] {
    ...
    public:
         ...
         */
        static const uint8_t *find_pattern_scalar(const uint8_t *const pattern, const uint32_t patLen,
         ...
         * @return first start of pattern in data, or \c nullptr if not found
         */
        static const uint8_t *find_pattern(const uint8_t *const pattern, const uint32_t patLen, const uint8_t *data,
            ...
            if constexpr (Arch::ESP32S3) {
                ...
                asm volatile("EE.VLD.128.IP q0, %[first], 16"
                             "\n"
